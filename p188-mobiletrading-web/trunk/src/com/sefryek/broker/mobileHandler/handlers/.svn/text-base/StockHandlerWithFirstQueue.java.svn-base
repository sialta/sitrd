 ...
 * Time: 11:02:14 AM
 */
public class StockHandlerWithFirstQueue extends MIDletHandler {
    private static Logger logger = Logger.getLogger(LoginHandler.class);
    private TradingMessage tradingMessage;

    public StockHandlerWithFirstQueue(TradingMessage tradingMessage) {
    ...
    public TradingMessage handleRequest() throws Exception {
        ...
        if (userSession != null) {
            responseBody = MarketManager.getStockPriceWithFirstQueue(tradingMessage, stockCode);
