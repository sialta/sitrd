 ...
 * Time: 17:28:12 PM
 */
public class StockBasketHandler extends MIDletHandler {

    private static Logger logger = Logger.getLogger(StockBasketHandler.class);
    private TradingMessage tradingMessage;

    public StockBasketHandler(TradingMessage tradingMessage) {
    ...
    public TradingMessage handleRequest() throws Exception {
        ...
        if (userSession != null) {
            responseBody = userSession.getManager().getCustomerCdsPortfolio(tradingMessage, brokerCode, customerCode);
