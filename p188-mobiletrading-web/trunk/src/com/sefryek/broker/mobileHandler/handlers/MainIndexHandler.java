 ...
 * Time: 1:49:56 PM
 */
public class MainIndexHandler extends MIDletHandler {

    private static Logger logger = Logger.getLogger(MainIndexHandler.class);
    private TradingMessage tradingMessage;

    public MainIndexHandler(TradingMessage tradingMessage) {
        this.tradingMessage = tradingMessage;
    }

    public TradingMessage handleRequest() throws Exception {
        logger.debug("MainIndex handle request.");
        ...
        if(userSession != null) {
            responseBody = MarketManager.mainIndexService(tradingMessage);
